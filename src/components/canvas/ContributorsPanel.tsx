...
export default function ContributorsDropdown({
  ...
  const contributors = useSelector(selectProjectContributors) as
    | Array<{
        _id?: string;
        fullName?: string;
        email?: string;
  ...
  const isLoading = useSelector(
    (state: RootState) => state.projects.loading.contributors,
  );
      ...
      (contributors || []).map((c: any) =>
        ...
        typeof c === "string"
          ...
          : // If it's an object, normalize its properties
            {
              id: c._id ?? "",
              name: c.fullName ?? "Unknown Contributor",
  ...
  useEffect(() => {
    if (currentProject?._id)
      dispatch(fetchContributors({ projectId: currentProject?._id }));
  ...
  const handleRemove = async (id: string, name: string) => {
    try {
      ...
      toast.error(
        `Failed to remove contributor: ${err?.message || "Unknown error"}`,
      );
    } finally {
      setConfirmUser(null);
      setLoading(false);
  ...
  return (
    <div
      ref={dropdownRef}
      className="relative w-full mb-5 border-[1px] border-[#ffffff]"
    >
      <Button
        variant="outline"
        size="sm"
      ...
      >
        ...
        <ChevronDown
          className={`h-4 w-4 opacity-70 text-[#ffffff] transition-transform ${isOpen ? "rotate-180" : ""}`}
        />
      </Button>

      {/* STEP 3: Conditional Dropdown Content */}
      {isOpen && (
        <div className="absolute top-full mt-2 w-full z-20 bg-[#141414] border-1 border-[#ffffff] rounded-lg shadow-xl text-[#] font-[Georgia, serif] p-1">
          ...
          {isLoading ? (
            ...
            <Command shouldFilter={false}>
              ...
              {filteredUsers.length === 0 ? (
                <CommandEmpty className="py-8 text-muted-foreground text-center text-[#ffffff]">
                  No contributors yet.
                </CommandEmpty>
              ) : (
                  ...
                  <div className="overflow-y-auto max-h-64">
                    {filteredUsers.map((c) => {
                      ...
                      return (
                        <CommandItem
                          key={c.id || c.name}
                          className="flex w-full items-center text-[#ffffff] justify-between gap-2 px-3 py-2"
                        >
                          {/* User info */}
                          ...
                          <Button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              if (c.id)
                                setConfirmUser({ id: c.id, name: c.name });
                            }}
                            title={
                              isSelf
        ...
        <AlertDialogContent className="bg-[#0F0D0D] border border-white/10 text-white !font-sans rounded-[12px] shadow-2xl max-w-[500px] p-8">
          <AlertDialogHeader className="gap-2">
            ...
            <AlertDialogDescription className="text-center text-[#E0E0E0] text-base leading-relaxed pt-2">
              This will remove{" "}
              <strong className="text-white">{confirmUser?.name}</strong> from
              the project. They will no longer be able to contribute.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="flex gap-4 sm:justify-center mt-6 w-full">
            ...
            <Button
              ...
              onClick={() =>
                confirmUser && handleRemove(confirmUser.id, confirmUser.name)
              }
              disabled={loading}
            >
