...
const ContributionSidebar = ({
  ...
  // --- ADD THIS STATE ---
  const [isContributorsDropdownOpen, setIsContributorsDropdownOpen] =
    useState(false);
  ...
  const handleCreateNewContribution = async () => {
    ...
    try {
      await dispatch(
        createEmptyContribution({ projectId, userId: user?._id }),
      ).unwrap();
      ...
      toast.error(
        err.response?.data?.message || "Failed to create contribution.",
      );
    } finally {
  ...
  return (
    <div
      ref={sidebarRef}
      className={`fixed  top-0 right-0 h-screen flex items-center z-[9999]  `}
      style={{
    ...
    >
      ...
      {
        <div
          className={`w-[350px] h-screen bg-[#1D1D1D] shadow-2xl transform transition-transform duration-300 ease-in-out  flex flex-col ${
        ...
        >
          ...
          >
            <p className="text-[14px] font-medium text-[#ffffff] mb-4 ">
              If a contribution receives over 50% downvotes from all project
              contributors it will be rejected and permanently deleted from the
              canvas...
            </p>
            ...
            <div className=" text-sm mb-1">
              {user?._id && currentProject?.status !== "Completed" && (
                <Button
                  ...
                  title={
                    !isContributor
                      ...
                      : isLimitReached
                        ? `You have reached the limit of ${MAX_CONTRIBUTIONS_LIMIT} contributions.`
              ...
              {isLimitReached && (
                <p className="text-xs text-red-600 text-center mt-2">
                  Contribution limit reached ({userContributionCount}/
                  {MAX_CONTRIBUTIONS_LIMIT}).
                </p>
              )}
            </div>
