...
const CreateProjectForm = () => {
  const [formData, setFormData] = useState({
    title: '',
    description: '',
    // CORRECTED: Default size now matches the TiledCanvas viewport
    width: 1024,
    height: 1024,
    palette: '',
    thumbnailUrl: '',
  ...
  // CORRECTED: Preset sizes now align with the 1024x1024 viewport
            ...
            <div className={`${currentStep === 2 ? 'block' : 'hidden'}`}>
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">{presetCanvasSizes.map((preset, index) => (<button key={index} type="button" onClick={() => setFormData(prev => ({ ...prev, width: preset.width, height: preset.height }))} className="p-3 text-sm border-2 border-gray-200 rounded-xl hover:border-amber-500 hover:bg-amber-50 transition-all duration-300 bg-white/70 backdrop-blur-sm"><div className="font-medium">{preset.name}</div><div className="text-gray-500 text-xs">{preset.width} × {preset.height}</div></button>))}</div>
                </div>
                <div className="grid grid-cols-2 gap-6">
                  <div><Label htmlFor="width" className="text-[#5d4037] font-medium flex items-center space-x-2 mb-3"><Monitor className="w-4 h-4" /><span>Canvas Width (px)</span><span className="text-red-500">*</span></Label><Input id="width" type="number" name="width" value={formData.width} onChange={handleChange} className="h-12 border-2 border-gray-200 rounded-xl focus:border-amber-500 focus:ring-2 focus:ring-amber-200 transition-all duration-300 bg-white/50 backdrop-blur-sm" required /></div>
                  <div><Label htmlFor="height" className="text-[#5d4037] font-medium flex items-center space-x-2 mb-3"><Monitor className="w-4 h-4" /><span>Canvas Height (px)</span><span className="text-red-500">*</span></Label><Input id="height" type="number" name="height" value={formData.height} onChange={handleChange} className="h-12 border-2 border-gray-200 rounded-xl focus:border-amber-500 focus:ring-2 focus:ring-amber-200 transition-all duration-300 bg-white/50 backdrop-blur-sm" required /></div>
                </div>
                <div>
                  ...
                  <div className="mb-4"><p className="text-sm text-[#8d6e63] mb-2">Popular Colors:</p><div className="flex flex-wrap gap-2">{popularColors.map((color) => (<button key={color} type="button" onClick={() => handleColorAdd(color)} className="w-8 h-8 rounded-lg border-2 border-white shadow-md hover:scale-110 transition-transform duration-200" style={{ backgroundColor: color }} title={color} />))}</div></div>
                  {colorPalette.length > 0 && (<div className="mb-4"><p className="text-sm text-[#8d6e63] mb-2">Selected Colors:</p><div className="flex flex-wrap gap-2">{colorPalette.map((color, index) => (<div key={index} className="relative group"><div className="w-10 h-10 rounded-lg border-2 border-white shadow-md" style={{ backgroundColor: color }} /><button type="button" onClick={() => removeColor(color)} className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200"><X className="w-3 h-3" /></button></div>))}</div></div>)}
            ...
            <div className={`${currentStep === 3 ? 'block' : 'hidden'}`}>
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="flex space-x-3"><Input id="thumbnailUrl" name="thumbnailUrl" value={formData.thumbnailUrl} onChange={handleChange} className="h-12 border-2 border-gray-200 rounded-xl focus:border-amber-500 focus:ring-2 focus:ring-amber-200 transition-all duration-300 bg-white/50 backdrop-blur-sm" placeholder="https://... or upload" required /><input type="file" ref={thumbnailInputRef} name="thumbnailUrl" onChange={handleFileChange} className="hidden" accept="image/*" /><Button type="button" variant="outline" onClick={() => thumbnailInputRef.current?.click()} className="h-12 px-4 border-2 border-gray-200 rounded-xl hover:border-amber-500 hover:bg-amber-50 transition-all duration-300"><Upload className="w-4 h-4 mr-2" /> Upload</Button></div>
                ...
                <div>
                  ...
                  <div className="flex space-x-3"><Input id="baseImageUrl" name="baseImageUrl" value={formData.baseImageUrl} onChange={handleChange} className="h-12 border-2 border-gray-200 rounded-xl focus:border-amber-500 focus:ring-2 focus:ring-amber-200 transition-all duration-300 bg-white/50 backdrop-blur-sm" placeholder="https://... or upload" /><input type="file" ref={baseImageInputRef} name="baseImageUrl" onChange={handleFileChange} className="hidden" accept="image/*" /><Button type="button" variant="outline" onClick={() => baseImageInputRef.current?.click()} className="h-12 px-4 border-2 border-gray-200 rounded-xl hover:border-amber-500 hover:bg-amber-50 transition-all duration-300"><Upload className="w-4 h-4 mr-2" /> Upload</Button></div>
                ...
                <div>
                  ...
                  <Input id="targetCompletionDate" type="date" name="targetCompletionDate" value={formData.targetCompletionDate} onChange={handleChange} className="h-12 border-2 border-gray-200 rounded-xl focus:border-amber-500 focus:ring-2 focus:ring-amber-200 transition-all duration-300 bg-white/50 backdrop-blur-sm" />
                </div>
              </div>
            </div>

            <div className="flex justify-between items-center pt-8 border-t border-gray-200">
              <Button type="button" variant="outline" onClick={handlePreviousStep} disabled={currentStep === 1} className="h-12 px-8 border-2 border-gray-200 rounded-xl hover:border-amber-500 hover:bg-amber-50 transition-all duration-300 disabled:opacity-50">Previous</Button>
              <div className="flex space-x-4">
                {currentStep < 3 ? (
                  <Button type="button" onClick={handleNextStep} className="h-12 px-8 bg-gradient-to-r from-[#d4af37] to-[#5d4037] hover:from-[#d4af37] hover:to-[#212121] text-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300">Next</Button>
                ) : (
                  <Button type="button" onClick={handleSubmit} disabled={isSubmitting} className="h-12 px-8 bg-gradient-to-r from-[#3e2723] to-[#5d4037] cursor-pointer hover:opacity-75 text-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 disabled:opacity-70">{isSubmitting ? (<div className="flex items-center"><div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin mr-2"></div>Creating...</div>) : (<div className="flex items-center"><Sparkles className="w-4 h-4 mr-2" />Create Project</div>)}</Button>
                )}
              </div>
            </div>
          </div>
